...
exports.hollywoodHandler = void 0;
...
const hollywoodHandler = async (req, res) => {
    try {
        const response = await axios_1.default.get(HOLLYWOOD_API_URL);
        if (!response.data || !Array.isArray(response.data.results)) {
            return res.status(500).json({ message: "Invalid data structure received from the Hollywood API." });
        }
        const hollywood = response.data.results.map((movie) => ({
            movie_id: movie.id,
            title: movie.title,
...
exports.hollywoodHandler = hollywoodHandler;
//# sourceMappingURL=hollywoodHandler.js.map